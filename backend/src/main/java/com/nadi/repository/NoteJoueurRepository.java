...
public interface NoteJoueurRepository extends JpaRepository<NoteJoueur, Long> {
    ...
    @Query("SELECT AVG(COALESCE(n.tactique, n.physique)) FROM NoteJoueur n WHERE n.joueur.id = :joueurId")
    BigDecimal findAverageTactiqueByJoueur(@Param("joueurId") Long joueurId);

    @Query("SELECT AVG(COALESCE(n.mental, n.technique)) FROM NoteJoueur n WHERE n.joueur.id = :joueurId")
    BigDecimal findAverageMentalByJoueur(@Param("joueurId") Long joueurId);

    @Query("SELECT AVG(COALESCE(n.endurance, n.physique)) FROM NoteJoueur n WHERE n.joueur.id = :joueurId")
